        ...
        <ul className='hidden gap-5 font-medium lg:flex xl:gap-12'>
          {mainmenu.map((item, index) => (
            ...
            >
              <Link
                href={item.link}
                className={`flex font-medium  text-white cursor-pointer items-center text-lg duration-200 ease-in-out hover:text-primary group-hover:text-primary  ${isActive === item.link ? 'text-primary' : 'text-black/60'}`}
              >
                {item.name}
              </Link>
            </li>
            ...
            <nav className='flex flex-1 justify-center overflow-auto'>
              <ul className='flex flex-1 flex-col items-start justify-start divide-y divide-white/10 px-5'>
                {mainmenu.map((item, index) => (
                  ...
                  >
                    <Link
                      className={`relative inline-flex flex-1 justify-between py-2 text-sm font-medium  duration-200 ease-in-out xl:text-base ${isActive === item.link ? 'text-white' : 'text-white/70 hover:text-secondary'}`}
                      href={item.link}
                    >
                      <span>{item.name}</span>
                    </Link>
