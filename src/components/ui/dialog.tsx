...
>(({ className, children, hideClose, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ...
      {...props}
    >
      <div className='max-h-[calc(100vh-64px)] overflow-y-auto p-3'>
        {children}
      </div>
      {hideClose && (
        <DialogPrimitive.Close className='absolute right-4 top-4 rounded-sm text-white opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground'>
